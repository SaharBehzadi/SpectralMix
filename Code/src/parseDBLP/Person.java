...
public class Person {
    ...
    int[] clid; //for method getCoauthors: cluster id of node
    String[] nn;
    ...
    private boolean contains(String bla, String[] names) {
        boolean res = false;
        for (int i = 0; i < names.length; i++) {
            if (names[i].equals(bla)) {
                res = true;
            }
        }
        return res;
    ...
    public Graph getCoauthors(String[] names) {
        Graph<Integer, Integer> g = new UndirectedSparseGraph<Integer, Integer>();
        int edgeCount = 0;
        HashMap<String, Integer> nodeLabels = new HashMap<String, Integer>();
        Vector<String> nodes = new Vector<String>();
        Vector<Integer> id = new Vector<Integer>();
        int nodeCount = 0;
        //add all names and their co-authors with edges: with names[]
        for (int i = 0; i < names.length; i++) {
            Person p = searchPerson(names[i]);
            if (!(nodes.contains(names[i]))) {
                nodeLabels.put(p.name, nodeCount);
                nodes.add(p.name);
                id.add(i);
                nodeCount++;
                g.addVertex(nodeCount);
            }

            Person[] co = p.getCoauthors();
            for (int j = 0; j < co.length; j++) {
                //do not include the big fish
                // if (!(nodes.contains(co[j].name)) && !(contains(co[j].name, names))) {
                //include the big fish
                if (!(nodes.contains(co[j].name))) {
                    nodeLabels.put(co[j].name, nodeCount);
                    nodes.add(co[j].name);
                    id.add(i);
                    g.addVertex(nodeCount);
                    nodeCount++;
                    g.addEdge(edgeCount, nodeLabels.get(p.name), nodeLabels.get(co[j].name));
                    edgeCount++;
                }
            }
        }
        System.out.println(g.getVertexCount() + " vertices " + g.getEdgeCount() + " edges");
        //add all missing edges between the nodes



        for (int i = 0; i < nodes.size(); i++) {
            if(nodes.elementAt(i).equalsIgnoreCase("Claudia Plant"))
                System.out.println("m");
            Person p = (Person) personMap.get(nodes.elementAt(i));
            Person[] co = p.getCoauthors();
            for (int j = 0; j < co.length; j++) {
                if (g.containsVertex(nodeLabels.get(co[j].name))) {
                    if(!g.isNeighbor(nodeLabels.get(p.name), nodeLabels.get(co[j].name))){
                    g.addEdge(edgeCount, nodeLabels.get(p.name), nodeLabels.get(co[j].name));
                    edgeCount++;
                    }
                }
...
//            System.out.println(i + " " + nodes.elementAt(i));
//        }
//        System.out.println("-----------------");
////DEBUG

        Collection vv = nodeLabels.entrySet();
        Iterator<Entry<String, Integer>> itt = vv.iterator();
        while (itt.hasNext()) {
            Entry<String, Integer> next = itt.next();
            System.out.println(next.getKey() + " " + next.getValue());
            //(<Entry<String,Integer>>) itt.next();

        }
        clid = new int[id.size()];
        for (int i = 0; i < clid.length; i++) {
            clid[i] = id.elementAt(i);
        }

        this.nn = new String[nodes.size()];
        for (int i = 0; i < nodes.size(); i++) {
            nn[i] = nodes.elementAt(i);
        }
        return g;
    }
