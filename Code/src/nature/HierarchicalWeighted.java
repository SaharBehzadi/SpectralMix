 ...
 */
public class HierarchicalWeighted extends AbstractLayout<Integer, Integer> implements IterativeContext {
    ...
    public HierarchicalWeighted(Graph<Integer, Integer> g) {
        super(g);
        this.g = g;
        r = new Random(20);
        //this.factor = factor;
    ...
    public HierarchicalWeighted(Graph<Integer, Integer> g, double[][] coord, double[] nodeWeights, double[] edgeMultiplicities) {
        super(g);
        this.g = g;
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
    ...
    public HierarchicalWeighted(Graph<Integer, Integer> g, double factor) {
        super(g);
        r = new Random(20);
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
...
//            //TEST  compute sigmoid: simple
//             Pair[] p = new Pair[m];
//             WeightedPair[] pp = new WeightedPair[m]; 
//            for (int i = 1; i < n; i++) {
//                for (int j = 0; j < i; j++) {
//                    double alpha = 0.0;
//                    double beta = 0.0;
...
//                    }
//
//                    p[i * (i - 1) / 2 + j] = new Pair(dist(coord[i], coord[j]), graph.isNeighbor(i, j));
//                    pp[i * (i - 1) / 2 + j] = new WeightedPair(dist(coord[i], coord[j]), alpha, beta);
//                }
//            }
//            SimpleSigmoid s = new SimpleSigmoid(p);
//            WeightedSigmoid sw = new WeightedSigmoid(pp);
...
//            System.out.println(loop);
             ...
             //SimpleSigmoid si = new SimpleSigmoid(p);
            if (s.sigma > 1) {
                s.sigma = 1;
            }
            ...
            for (int i = 0; i < g.getVertexCount(); i++) {
                ...
                beta = nodeWeights[i] * nodeWeights[j];   
                     ...
                     alpha = edgeMultiplicities[g.findEdge(i, j)];
                    beta = nodeWeights[i] * nodeWeights[j] - alpha;
                }
                    //double[] dwalt = si.parabola(x, graph.isNeighbor(i, j));
                    double[] dw = WeightedSigmoid.dwFormula(x, s.mu, s.sigma, alpha, beta);
                    // double[] dw = si.parabola(x, graph.isNeighbor(i, j));
//                    if(Double.isNaN(dw[0]) || Double.isNaN(dw[1]) || Double.isInfinite(dw[1])){
//                        System.out.println(currentIteration);
//                        dwalt = si.parabola(x, graph.isNeighbor(i, j));
//                        dw = WeightedSigmoid.dwFormula(x, s.mu, s.sigma, alpha, beta);
//                    }
//                    if(Math.abs((dwalt[0]-dw[0])/dwalt[0])>1e-1 || Math.abs((dwalt[1]-dw[1])/dwalt[1])>1e-1){
//                        
//                        //System.out.println("*");
//                    }
