...
import nature.util.IndexPair;

/**
 * 22.12.15: Weighted mit Grid
 *
 */
public class HierarchicalWeightedGrid {
    ...
    double maxSigma;
    double[] nodeWeights;
    double[] edgeMultiplicity;
    boolean verbose = true;
    static int maxIteration = 2000;

    public HierarchicalWeightedGrid(Graph g) {
        this.g = g;
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
    ...
    public HierarchicalWeightedGrid(Graph g, double maxSigma) {
        this.g = g;
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
        this.maxSigma = maxSigma;
    ...
    public HierarchicalWeightedGrid(Graph g, double maxSigma, double[] nodeWeights, double[] edgeMultiplicity) {
        this.g = g;
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
        this.maxSigma = maxSigma;
    ...
    public void run(double[][] coord, double convConst) {
        ...
        while ((!converged || currentIteration < 100) && currentIteration < maxIteration) {
            //while (!converged) {
            //DEBUG
...
//                System.out.println("m");
            ...
            if (verbose) {
                System.out.println(currentIteration + " " + aktCost + " " + converged + " " + sigma);
            }
        }
        if (verbose) {
    ...
    public void init() {
        ...
        gk.setCoord(coord);
        gk.setEdgeMultiplicities(edgeMultiplicity);
        gk.setNodeWeights(nodeWeights);

        // = 0;
        bestCost = gk.mdlFunctionWeightedSigmoid(maxSigma);
    ...
    private void checkNan(double[][] d) {
        Vector<IndexPair> nan = new Vector<IndexPair>();
        for (int i = 0; i < d.length; i++) {
            for (int j = 0; j < d[i].length; j++) {
                if (Double.isNaN(d[i][j])) {
                    nan.add(new IndexPair(i, j));
                }
            }
        }
        if (nan.size() > 0) {
            System.out.println(d.toString() + " " + nan.size());
        } else {
            System.out.println(d.toString() + " check ok");
        }
...
//        //compute sigmoid: simple
//        Pair[] p = new Pair[m];
//        for (int i = 1; i < n; i++) {
//            for (int j = 0; j < i; j++) {
//                p[i * (i - 1) / 2 + j] = new Pair(dist(coord[i], coord[j]), g.isNeighbor(i, j));
//                //DEBUG
...
//                } else {
                ...
                p[i * (i - 1) / 2 + j] = new WeightedPair(dist(coord[i], coord[j]), alpha, beta);
            }
        }
        WeightedSigmoid s = new WeightedSigmoid(p);
                ...
                for (int a = 0; a < pId[i][j].length; a++) {
                    for (int b = 0; b < a; b++) {
                        ...
                        if (!g.isNeighbor(pId[i][j][a], pId[i][j][b])) {
                            ...
                            //alpha = edgeMultiplicity[g.findEdge(i, j)]; // should be 0
                            beta = nodeWeights[pId[i][j][a]] * nodeWeights[pId[i][j][b]];
                            double[] dw = WeightedSigmoid.dwFormula(x, s.mu, s.sigma, alpha, beta);
                            if ((Double.isNaN(dw[0]) || Double.isNaN(dw[1])) && currentIteration == 215) {  //here is the problem
                    ...
                    for (int a = 0; a < pId[i][j].length; a++) {
                        for (int b = 0; b < pId[i + 1][j].length; b++) {
                            ...
                            if (!g.isNeighbor(pId[i][j][a], pId[i + 1][j][b])) {
                                //double[] dw = s.parabola(x, false);
                                //alpha = edgeMultiplicity[g.findEdge(i, j)]; // should be 0
                                beta = nodeWeights[pId[i][j][a]] * nodeWeights[pId[i + 1][j][b]];
                                double[] dw = WeightedSigmoid.dwFormula(x, s.mu, s.sigma, alpha, beta);
                                if ((Double.isNaN(dw[0]) || Double.isNaN(dw[1])) && currentIteration == 215) {
                                    System.out.println(currentIteration);
                                }
                        ...
                        for (int a = 0; a < pId[i][j].length; a++) {
                            for (int b = 0; b < pId[i2][j + 1].length; b++) {
                                ...
                                if (!g.isNeighbor(pId[i][j][a], pId[i2][j + 1][b])) {
                                    //double[] dw = s.parabola(x, false);
                                    alpha = edgeMultiplicity[g.findEdge(i, j)];
                                    beta = nodeWeights[pId[i][j][a]] * nodeWeights[pId[i2][j + 1][b]];
                                    double[] dw = WeightedSigmoid.dwFormula(x, s.mu, s.sigma, alpha, beta);
                                    if ((Double.isNaN(dw[0]) || Double.isNaN(dw[1])) && currentIteration == 215) {
                                        System.out.println(currentIteration);
                                    }
