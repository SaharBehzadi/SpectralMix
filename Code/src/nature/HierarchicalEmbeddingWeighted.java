 ...
 *
 */
public class HierarchicalEmbeddingWeighted {
    ...
    public HierarchicalEmbeddingWeighted(Graph g, double seedPercentage) {
        this.g = g;
        Random r = new Random();
...
//                String filename = "coord_" + level + ".mat";
//                IO ea = new IO();
//                ea.writeDoubleToMatlab(coord, "coord", filename);
...
//            if (level == 5) {
//                Visualization v = new Visualization(red[level]);
//                double[][] res = v.getCoordinatesWeighted(coord, nodeWeights[level], edgeMultiplicities[level]);
//
//            }

            IO ea = new IO();
//            if (level == 23) {
//                String filename = "graphLevel" + level;
//                ea.writeGraphToMatlab(red[level], filename);
//                double[][] nodeWeightsn = new double[nodeWeights[level].length][1];
//                double[][] edgeMultiplicitiesn = new double[edgeMultiplicities[level].length][1];
//                for (int i = 0; i < nodeWeights[level].length; i++) {
//                    nodeWeightsn[i][0] = nodeWeights[level][i];
//                }
//                for (int i = 0; i < edgeMultiplicities[level].length; i++) {
//                    edgeMultiplicitiesn[i][0] = edgeMultiplicities[level][i];
//                }
//                DataUtils du = new DataUtils();
//                du.saveAsMatlab3(nodeWeightsn, edgeMultiplicitiesn, coord, "nodeweights", "edgemultiplicities", "coord", "nw" + level + ".mat");
//            }
            //gg.run();
            if (red[level].getVertexCount() > 1) {
                gg.run(coord, convConstant);
...
//        double srate = Math.sqrt((double) numObjNew / numObjOld);
