    ...
    public GempePar(Graph g) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i != j) {
                    for (int k = 0; k < d; k++) {
                        coord[i][k] += z[i][j][k] / (n - 1);
                    }
                }
            }
        }
        if(verbose){
           du.saveAsMatlab(coord, "init", "init.mat");
           System.out.println("initialization saved");
        }
        computeSigmoid();

    }

    public GempePar(Graph g, double maxSigma) {
        this.g = g;
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
...
//            }
    ...
    public void runParallel(int numT, int numUpdates) {
        int numInnerIter = 1000;
        for (int i = 0; i < numUpdates; i++) {
            try {
                updateParallel(numT, numInnerIter);
            } catch (InterruptedException ex) {
                Logger.getLogger(GempePar.class.getName()).log(Level.SEVERE, null, ex);
            }
            computeSigmoid();
        }
    ...
    public void updateParallel(int numT, int numIter) throws InterruptedException {
        //distribute edges over the threads
        ...
        for (int i = 0; i < numT - 1; i++) {
            ...
            GempeUpdateThread t = new GempeUpdateThread(numIter, edges, this, counter);
            collection.add(Executors.callable(t));
        }
...
//        System.out.println("init: coding Costs " + bestCost + " saved bits: " + savedBits);
    ...
    public void computeSigmoid() {
        ...
        if (verbose) {
            System.out.println("aktCost " + aktCost + " " + s.sigma);
        }
    }
