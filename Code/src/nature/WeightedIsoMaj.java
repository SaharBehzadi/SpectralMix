 ...
 */
public class WeightedIsoMaj extends AbstractLayout<Integer, Integer> implements IterativeContext {
    ...
    private double lastCost; //cost in previous interation
    ...
    public WeightedIsoMaj(Graph<Integer, Integer> g) {
        super(g);
        this.g = g;
        r = new Random(20);
        //this.factor = factor;
    ...
    public WeightedIsoMaj(Graph<Integer, Integer> g, double factor) {
        super(g);
        r = new Random(20);
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
    ...
    public void initialize() {
        ...
        if(mdsInit){
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < i; j++) {
                    if(g.isNeighbor(i, j))
                        distances[i][j] = distances[j][i] = 0.10;
                    else
                         distances[i][j] = distances[j][i] = 20.0;
                }
                coord = new Matrix(MDSJ.classicalScaling(distances)).transpose().getArrayCopy();
            }
