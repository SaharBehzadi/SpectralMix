 ...
 */
public class StochasticGDDispl extends AbstractLayout<Integer, Integer> implements IterativeContext {
    ...
    int[][] relevantNotLinks; //numNodes n x K: in each row: for each node the notLinks which are considered during the optimization
    ...
    public StochasticGDDispl(Graph<Integer, Integer> g) {
        super(g);
        this.g = g;
        r = new Random(20);
        //this.factor = factor;
    ...
    public StochasticGDDispl(Graph<Integer, Integer> g, double factor) {
        super(g);
        r = new Random(20);
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
...
//        
        ...
        gk.setCoord(coord);
        loop = 0;
        //sample for each Node numNotLinks nodes which are used in the objective function with prob. proportional to deg^0.75
        int numNotLinks = 5; //as used in the experiments of the LINE paper 
        relevantNotLinks = new int[n][numNotLinks];
        double[] samplingDist = new double[n];
        double sum = 0.0;
        for (int i = 0; i < samplingDist.length; i++) {
            samplingDist[i] = Math.pow(g.outDegree(i), 0.75);

        }
        DataUtils du = new DataUtils();
        samplingDist = du.scale(samplingDist);
        SortPack[] sd = SortPack.array(samplingDist);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < numNotLinks; j++) {
                double akt = r.nextDouble();
                int intervalStart = 0;
                while (sd[intervalStart].value < akt) {
                    intervalStart++;
                }
                int intervalEnd = intervalStart;
                while (sd[intervalEnd].value == sd[intervalStart].value && intervalEnd < sd.length-1){
                    intervalEnd++;
                }
               
                if (intervalStart != intervalEnd) {
                    int ii = r.nextInt((intervalEnd - intervalStart));
                    intervalStart += +ii;
                }
                int searchStart = intervalStart;
                while (g.isNeighbor(i, sd[intervalStart].index) && intervalStart < sd.length-1) {
                    intervalStart++;
                }
                if (intervalStart == sd.length - 1) {
                    intervalStart = searchStart;
                    while (g.isNeighbor(i, sd[intervalStart].index) && intervalStart >= 0) {
                        intervalStart--;
                    }
                }

                relevantNotLinks[i][j] = sd[intervalStart].index;
            }
        }
        double initialCost = gk.probCost(relevantNotLinks);
