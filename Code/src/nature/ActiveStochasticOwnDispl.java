 ...
 */
public class ActiveStochasticOwnDispl extends AbstractLayout<Integer, Integer> implements IterativeContext {
    ...
    int[] mostExpensiveNotLink; //save for each node the most expensive not link for active update
    Graph g;
    GraphCompression gk;
    SimpleSigmoid s;
    Random r;
    ...
    public ActiveStochasticOwnDispl(Graph<Integer, Integer> g) {
        super(g);
        this.g = g;
        r = new Random(20);
        //this.factor = factor;
    ...
    public ActiveStochasticOwnDispl(Graph<Integer, Integer> g, double factor) {
        super(g);
        r = new Random(20);
        n = g.getVertexCount();
        m = n * (n - 1) / 2;
...
//        
        ...
        PairIndex[] p = new PairIndex[m];
        for (int i = 1; i < n; i++) {
            for (int j = 0; j < i; j++) {
                p[i * (i - 1) / 2 + j] = new PairIndex(dist(coord[i], coord[j]), graph.isNeighbor(i, j), i, j);
            }
        }

        s = new SimpleSigmoid(p);
        mostExpensiveNotLink = s.costAllPairs(p, n);
        gk.setCoord(coord);
        loop = 0;
...
//                x = dist(coord[akt.getFirst()], coord[mostExpensiveNotLink[akt.getFirst()]]);
//                //double aktW = ((2 * s.mu - 2 * x) * (s.a - s.b)) / (Math.pow(Math.PI, 0.5) * Math.pow(s.sigma, 3) * Math.exp(Math.pow(s.mu - x, 2) / Math.pow(s.sigma, 2)));
//                dw = s.parabola(x, false);
//                dist_new[akt.getFirst()][mostExpensiveNotLink[akt.getFirst()]] = dist_new[mostExpensiveNotLink[akt.getFirst()]][akt.getFirst()] = dw[0];
//                weights_new[akt.getFirst()][mostExpensiveNotLink[akt.getFirst()]] = weights_new[mostExpensiveNotLink[akt.getFirst()]][akt.getFirst()] = dw[1];
//
//                x = dist(coord[akt.getSecond()], coord[mostExpensiveNotLink[akt.getSecond()]]);
//                //double aktW = ((2 * s.mu - 2 * x) * (s.a - s.b)) / (Math.pow(Math.PI, 0.5) * Math.pow(s.sigma, 3) * Math.exp(Math.pow(s.mu - x, 2) / Math.pow(s.sigma, 2)));
//                dw = s.parabola(x, false);
//                dist_new[akt.getSecond()][mostExpensiveNotLink[akt.getSecond()]] = dist_new[mostExpensiveNotLink[akt.getSecond()]][akt.getSecond()] = dw[0];
//                weights_new[akt.getSecond()][mostExpensiveNotLink[akt.getSecond()]] = weights_new[mostExpensiveNotLink[akt.getSecond()]][akt.getSecond()] = dw[1];
...
//                    sumWeights_old += weights[i][j];
            ...
            for(int i = 0; i < n; i++){
                 double x = dist(coord[i], coord[mostExpensiveNotLink[i]]); 
                double[] dw = s.parabola(x, false);
                dist_new[i][mostExpensiveNotLink[mostExpensiveNotLink[i]]] = dist_new[mostExpensiveNotLink[i]][i] = dw[0];
                weights_new[i][mostExpensiveNotLink[i]] = weights_new[mostExpensiveNotLink[i]][i] = dw[1];
            }
